<template>
  <dashboard-layout title="auth.users.info.title">
    <el-col :md="{span:16, offset:1}">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item :label="$t('attributes.email')" prop="email">
          <el-input disabled readonly v-model="form.email" />
        </el-form-item>
...
export default {
  ...
  created () {
    get('/users/info').then((rst) => { this.form = rst }).catch(this.$message.error)
  },
  computed: {
    rules () {
      return {
