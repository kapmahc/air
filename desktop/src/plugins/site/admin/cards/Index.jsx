  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        key: 'content',
        render: (text, record) => (<div>{record.title}<br/>{record.logo}<br/>{record.href}<br/>{record.type}<br/>{record.summary}<br/>{record.action}</div>)
      },
      {
    ...
    return (
      <Layout admin breads={[{href: '/admin/cards', label: 'site.admin.cards.index.title'}]}>
        <Row>
          <Col>
